        ...
        return (
            <AppShell pageTitle="Análise Detalhada" pageSubtitle="Carregando dados...">
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', padding: '60px 0' }}>
                    <div style={{
                        width: '48px', height: '48px', border: `4px solid ${C.green}`,
        ...
        <AppShell
            ...
            topBarContent={
                <div style={{
                    ...
                    backgroundColor: product.healthScore > 70 ? 'rgba(26, 92, 58, 0.1)' : 'rgba(214, 40, 40, 0.1)',
                    color: product.healthScore > 70 ? C.green : C.redMid,
                ...
                <div style={{ gridColumn: 'span 8', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    <div style={{ padding: '32px', backgroundColor: '#FFFFFF', border: '1px solid #E5E7EB', borderRadius: '24px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px' }}>
                            <Scale size={20} color={C.green} />
                            <h3 style={{ fontSize: '20px', fontWeight: '700', color: C.ink, margin: 0 }}>Diagnóstico de Lucratividade</h3>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '48px' }}>
                            <div>
                                <p style={{ fontSize: '14px', fontWeight: '600', color: '#6B7280', marginBottom: '8px' }}>Margem Líquida Atual</p>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                    <h2 style={{ fontSize: '56px', fontWeight: '800', fontFamily: '"DM Mono"', color: currentMargin > 15 ? C.green : C.redMid, margin: 0 }}>
                                        {currentMargin.toFixed(1)}%
                                    </h2>
                                    {currentMargin > 15 ? <TrendingUp size={24} color={C.green} /> : <TrendingDown size={24} color={C.redMid} />}
                                </div>
                ...
                <div style={{ gridColumn: 'span 4', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    <div style={{ padding: '32px', backgroundColor: '#FFFFFF', border: '2px solid rgba(26, 92, 58, 0.2)', borderRadius: '24px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '24px' }}>
                            <div style={{ padding: '16px', background: C.paper, borderRadius: '12px' }}>
                                <p style={{ fontSize: '10px', color: C.inkMuted, textTransform: 'uppercase' }}>Nova Margem</p>
                                <p style={{ fontSize: '18px', fontWeight: '800', color: simMargin > 15 ? C.green : C.redMid }}>{simMargin.toFixed(1)}%</p>
                            </div>
                            <div style={{ padding: '16px', background: C.paper, borderRadius: '12px' }}>
                                <p style={{ fontSize: '10px', color: C.inkMuted, textTransform: 'uppercase' }}>Equilíbrio</p>
                                <p style={{ fontSize: '18px', fontWeight: '800', color: C.ink }}>{simBreakEven} un</p>
                            </div>
                        </div>
                        <button
                            onClick={() => navigate('/precificar/' + id)}
                            style={{ width: '100%', padding: '16px', background: C.green, color: C.white, border: 'none', borderRadius: '12px', fontWeight: '700', cursor: 'pointer' }}
                        >
                            Aplicar Novo Preço
                        </button>
                    </div>
...
function InsightItem({ title, text, type }) {
    const color = type === 'success' ? C.green : C.orange;
    const bgColor = type === 'success' ? 'rgba(26, 92, 58, 0.05)' : 'rgba(245, 158, 11, 0.05)';
    return (
        <div style={{ padding: '16px', background: bgColor, borderRadius: '12px', border: `1px solid ${color}22` }}>
            <h5 style={{ fontSize: '13px', fontWeight: '700', margin: 0, color: color }}>{title}</h5>
            <p style={{ fontSize: '12px', color: C.inkLight, margin: '4px 0 0' }}>{text}</p>
        </div>
    );
}
